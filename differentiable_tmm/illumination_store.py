...
def Get_illumination(item_name, value):
    
    value=np.copy(value)
    
    wavelength=np.array(list(illumination[item_name].keys()))
    illu=np.array(list(illumination[item_name].values()))
    wv_range=[wavelength.min(),wavelength.max()]
    fn=interp1d(wavelength, illu, kind='linear')
    
    value[value<wv_range[0]]=wv_range[0]
    value[value>wv_range[1]]=wv_range[1]
    
    return fn(value)



# Visual_range=np.arange(360,781)

# for i in illumination:
#     wavelength=np.array(list(illumination[i].keys()))
#     illu=np.array(list(illumination[i].values()))
#     wv_range=[wavelength.min(),wavelength.max()]
#     fn=interp1d(wavelength, illu, kind='linear')
    
#     value=np.copy(Visual_range)
#     value[value<wv_range[0]]=wv_range[0]
#     value[value>wv_range[1]]=wv_range[1]
    
#     illumination[i]=fn(value)
